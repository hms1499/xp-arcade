...
import type { GameDef } from "@/lib/game-registry";
...
export async function scoreMetadataResponse(
  req: Request,
  params: Promise<{ id: string }>,
  options: {
    game: GameDef;
    gameName: string;
    descriptionGameName: string;
  ...
  const ip =
    ...
    "anon";
  const rl = rateLimit(`${options.rateLimitPrefix}:${ip}`, RL_LIMIT, RL_WINDOW_MS);
  if (!rl.ok) {
  ...
  try {
    const res = await fetchCallReadOnlyFunction({
      ...
      contractAddress: options.game.contractAddress,
      contractName: options.game.contractName,
      functionName: "get-score-data",
      functionArgs: [uintCV(tokenId)],
      senderAddress: options.game.contractAddress,
    });
    ...
    return NextResponse.json(
      {
        name: `${options.gameName} Score #${tokenId}`,
        description: `On-chain proof of a ${options.descriptionGameName} game score: ${v.score}.`,
