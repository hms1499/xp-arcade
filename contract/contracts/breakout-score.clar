...
;; summary: XP Arcade XP Bricks on Stacks - score + trophy NFTs v2

(impl-trait .nft-trait.nft-trait)

(define-non-fungible-token breakout-score uint)
...
(define-public (mint-score (score uint) (player-name (string-ascii 24)))
  (let ((new-id (+ (var-get last-token-id) u1))
    ...
    (try! (nft-mint? breakout-score new-id tx-sender))
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? breakout-score token-id)))
...
;; --- Trophy NFT ---
(define-non-fungible-token breakout-trophy uint)
...
(define-data-var base-uri (string-ascii 80) "https://xp-snake.example/api/metadata/breakout/")
...
(define-read-only (get-trophy-owner (trophy-id uint))
  (ok (nft-get-owner? breakout-trophy trophy-id)))
  ...
  (let
    ...
    (let
      ...
      (let ((rank (+ u1 (get higher result)))
        ...
        (try! (nft-mint? breakout-trophy new-id tx-sender))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? breakout-score token-id sender recipient)))
