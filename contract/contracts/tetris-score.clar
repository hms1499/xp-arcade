;; title: tetris-score
;; summary: XP Arcade Tetris on Stacks - score + trophy NFTs v2

(impl-trait .nft-trait.nft-trait)

(define-non-fungible-token tetris-score uint)
...
(define-read-only (get-owner (token-id uint))
  (ok (nft-get-owner? tetris-score token-id)))
...
;; --- Trophy NFT ---
(define-non-fungible-token tetris-trophy uint)
...
(define-data-var base-uri (string-ascii 80) "https://xp-snake.example/api/metadata/tetris/")
...
(define-read-only (get-trophy-owner (trophy-id uint))
  (ok (nft-get-owner? tetris-trophy trophy-id)))
  ...
  (let
    ...
    (let
      ...
      (let ((rank (+ u1 (get higher result)))
        ...
        (try! (nft-mint? tetris-trophy new-id tx-sender))
...
(define-public (transfer (token-id uint) (sender principal) (recipient principal))
  (begin
    ...
    (nft-transfer? tetris-score token-id sender recipient)))
